...
def call_subprocess(args, **kwargs):
    ...
    res = subprocess.call(args, stderr=stderr_fh, **kwargs)
    stderr_fh.seek(0)
    if res != 0:
    ...
    for filename, file_path in listfiles(input_path):
        ...
        if mimetype.startswith("image/"):
            ...
            normalized_filename = video_template.format(
                item_name=item_name, asset_id=asset_id, extension=extension
            )
...
def generate_video_derivatives(
    input_path, poster_filename_template, output_path, overwrite, show_skipped
):
...
def generate_derivatives(
    context_file, input_path, output_path=None, overwrite=False, show_skipped=False
):
    # Assert that we can handle all the files in input_path.
    assert_no_unhandled_mimetypes(input_path)
    ...
    generate_video_derivatives(
        input_path, video_poster_filename_template, output_path, overwrite, show_skipped
    )
...
def add_to_project(
    ...
    print(
        f"Added {len(images)} images and {len(videos)} videos to " f'"{project_name}"'
    )
...
if __name__ == "__main__":
    ...
    if args.action == "auto":
        ...
        normalize_item_filenames(
            args.context_file, args.input_path, args.item_name, args.output_path
        )
