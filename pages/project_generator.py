...
def get_meta_tags(context):
    ...
    width = min(image["sources"]["original"].width, context.fallback_image_width)
...
def get_microdata_meta(context):
    ...
    if "operating_system" in project:
        tags.append(MDMeta(md.Props.operatingSystem, project["operating_system"]))
    return tags
...
def Nav(context):
    ...
    name_url_pairs = [
        (project["name"], project["slug"]) for project in context.projects
    ]
