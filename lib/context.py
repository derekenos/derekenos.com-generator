...
ImageSource = namedtuple(
    "ImageSource", ("filename", "mimetype", "url", "width", "height", "last_modified")
)

VideoSource = namedtuple(
    "VideoSource", ("filename", "mimetype", "url", "last_modified", "poster_url")
)
...
class Context:
    ...
    def is_large_static_storable(self, path):
        ...
        return os.stat(path).st_size / 1024 >= self.STATIC_LARGE_FILE_THRESHOLD_KB
...
def get_fallback_image_source(context, original_width, aspect, item_name, asset_id):
...
def normalize_projects(context, projects, all_tags):
    ...
    for name, project in name_project_map.items():
        ...
        # urltypes.
        for x in project.get("external_link_prop_type_name_urltype_url_tuples", ()):
        ...
        for k in (DEPENDS_ON, DEPENDENT_OF):
            ...
            for other_name in project[k]:
                ...
                if (
                    COLLATERAL_CREATIONS in other_project
                    and k == DEPENDENT_OF
    ...
    error_strs = [f"invalid {k}: {list(v)}" for k, v in invalid_d.items() if v]
    if error_strs:
...
def normalize_context(context):
    ...
    if not all_tags:
        raise InvalidContext(
            "context must define an all_tags array that "
